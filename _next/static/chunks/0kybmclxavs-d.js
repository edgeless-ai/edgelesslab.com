(globalThis.TURBOPACK||(globalThis.TURBOPACK=[])).push(["object"==typeof document?document.currentScript:void 0,43875,e=>{"use strict";var t=e.i(74092),a=e.i(43041),s=e.i(63560);let o=(0,e.i(42175).default)("arrow-right",[["path",{d:"M5 12h14",key:"1ays0h"}],["path",{d:"m12 5 7 7-7 7",key:"xquz4c"}]]);var r=e.i(29114),n=e.i(60409);function i(e,t){n.default.capture("cta_clicked",{cta_name:e,destination:t})}function l({text:e,className:a,delay:s=0}){let o=e.split(" ");return(0,t.jsxs)("span",{className:a,children:[o.map((e,a)=>(0,t.jsx)("span",{className:"inline-block mr-[0.25em]",style:{animation:`fadeInUp 0.5s cubic-bezier(0.16,1,0.3,1) ${s+.08*a}s both`},children:e},a)),(0,t.jsx)("style",{children:`
      ...
      `})]})}function h({children:e,className:a="",href:o,glowColor:r="var(--accent)"}){let n=(0,t.jsxs)("div",{className:`group relative overflow-hidden rounded-2xl border p-8 transition-all duration-200 hover:scale-[1.01] ${a}`,style:{background:"var(--bg-surface)",borderColor:"var(--border-subtle)"},children:[(0,t.jsx)("div",{className:"pointer-events-none absolute -inset-px rounded-2xl opacity-0 transition-opacity duration-300 group-hover:opacity-100",style:{background:`radial-gradient(600px circle at var(--mouse-x, 50%) var(--mouse-y, 50%), ${r}10, transparent 40%)`}}),(0,t.jsx)("div",{className:"pointer-events-none absolute inset-x-0 top-0 h-px opacity-0 transition-opacity duration-300 group-hover:opacity-100",style:{background:`linear-gradient(90deg, transparent, ${r}30, transparent)`}}),(0,t.jsx)("div",{className:"relative",children:e})]});return o?o.startsWith("/")?(0,t.jsx)(s.default,{href:o,children:n}):(0,t.jsx)("a",{href:o,target:"_blank",rel:"noopener noreferrer",children:n}):n}let d=["lorenz","rossler","spiral","flow"],u=[[235,85],[160,70],[270,75],[210,80],[330,65],[180,60]];function m(){let e=(0,a.useRef)(null),s=(0,a.useRef)(0),o=(0,a.useRef)(null),r=(0,a.useCallback)(e=>{var t;let a=(Date.now()^0xffffffff*Math.random())>>>0,s=(t=a,()=>{t|=0;let e=Math.imul((t=t+0x6d2b79f5|0)^t>>>15,1|t);return(((e=e+Math.imul(e^e>>>7,61|e)^e)^e>>>14)>>>0)/0x100000000}),r=d[Math.floor(s()*d.length)],n=u[Math.floor(s()*u.length)],i=e.width,l=e.height;o.current={particles:Array.from({length:600},()=>{let e=s()*i,t=s()*l;return{x:e,y:t,prevX:e,prevY:t,vx:(s()-.5)*2,vy:(s()-.5)*2,life:Math.floor(250*s())+80,maxLife:330,hueOffset:(s()-.5)*40}}),type:r,seed:a,rng:s,cx1:i*(.2+.6*s()),cy1:l*(.2+.6*s()),cx2:i*(.2+.6*s()),cy2:l*(.2+.6*s()),cx3:i*(.2+.6*s()),cy3:l*(.2+.6*s()),strength:.012+.025*s(),baseHue:n[0]+(s()-.5)*30,baseSat:n[1],hueRange:20+40*s(),trailFade:.03+.03*s(),time:0}},[]);return(0,a.useEffect)(()=>{let t=e.current;if(!t)return;let a=t.getContext("2d");if(!a)return;let n=0,i=0,l=()=>{let e=Math.min(window.devicePixelRatio||1,2),s=t.getBoundingClientRect();n=s.width,i=s.height,t.width=s.width*e,t.height=s.height*e,a.scale(e,e),r(t)};l();let c=()=>{let e=o.current;if(!e)return;e.time++,a.fillStyle=`rgba(0, 0, 0, ${e.trailFade})`,a.fillRect(0,0,n,i);let r=n/t.width,l=i/t.height;for(let s of e.particles){!function(e,t,a,s){e.prevX=e.x,e.prevY=e.y;let o=0,r=0,{type:n,cx1:i,cy1:l,cx2:c,cy2:h,cx3:d,cy3:u,strength:m,time:p}=t;switch(n){case"lorenz":{let t=i-e.x,a=l-e.y,s=c-e.x,n=h-e.y,p=Math.sqrt(t*t+a*a)+1,g=Math.sqrt(s*s+n*n)+1,f=d-e.x,y=u-e.y,w=Math.sqrt(f*f+y*y)+1;o=(t/p-n/g+f/w*.3)*m,r=(a/p+s/g-y/w*.3)*m;break}case"rossler":{let t=i-e.x,a=l-e.y,s=Math.sqrt(t*t+a*a)+1,n=.005*Math.sin(.002*p);o=(-a/s+t*(.008+n))*m,r=(t/s+a*(.008+n))*m;let d=c-e.x,u=h-e.y,g=Math.sqrt(d*d+u*u)+1;o+=u/g*m*.2,r-=d/g*m*.2;break}case"spiral":{let t=i-e.x,a=l-e.y,s=Math.sqrt(t*t+a*a)+1,n=Math.atan2(a,t)+.3+.2*Math.sin(.001*p);o=Math.cos(n)/s*m*8,r=Math.sin(n)/s*m*8;let d=c-e.x,u=h-e.y,g=Math.sqrt(d*d+u*u)+1,f=Math.atan2(u,d)-.5;o+=Math.cos(f)/g*m*3,r+=Math.sin(f)/g*m*3;break}case"flow":{let t=.002+5e-4*Math.sin(5e-4*p),n=Math.sin(e.x*t+.001*p)*Math.cos(e.y*t*.7),i=Math.cos(e.x*t*.8-8e-4*p)*Math.sin(e.y*t);o=n*m*1.5,r=i*m*1.5,o+=(.5*a-e.x)*1e-5,r+=(.5*s-e.y)*1e-5}}e.vx+=o,e.vy+=r,e.vx*=.982,e.vy*=.982,e.x+=e.vx,e.y+=e.vy,e.life--,e.life<=0&&(e.x=t.rng()*a,e.y=t.rng()*s,e.prevX=e.x,e.prevY=e.y,e.vx=(t.rng()-.5)*2,e.vy=(t.rng()-.5)*2,e.life=Math.floor(250*t.rng())+80,e.hueOffset=(t.rng()-.5)*40),e.x<-20&&(e.x=-20,e.vx*=-.5),e.x>a+20&&(e.x=a+20,e.vx*=-.5),e.y<-20&&(e.y=-20,e.vy*=-.5),e.y>s+20&&(e.y=s+20,e.vy*=-.5)}(s,e,t.width,t.height);let o=s.life/s.maxLife,c=o>.9?(1-o)*10:o<.2?5*o:1,h=Math.sqrt(s.vx*s.vx+s.vy*s.vy),d=c*Math.min(.8,.2+.15*h),u=.5+.4*h,m=e.baseHue+s.hueOffset+Math.sin(.003*e.time+s.hueOffset)*e.hueRange*.3,p=s.prevX*r,g=s.prevY*l,f=s.x*r,y=s.y*l,w=Math.abs(y-g);Math.abs(f-p)>.3*n||w>.3*i||(a.beginPath(),a.moveTo(p,g),a.lineTo(f,y),a.strokeStyle=`hsla(${m}, ${e.baseSat}%, 65%, ${d})`,a.lineWidth=u,a.lineCap="round",a.stroke(),h>2.5&&(a.beginPath(),a.arc(f,y,u+2,0,2*Math.PI),a.fillStyle=`hsla(${m}, ${e.baseSat}%, 70%, ${.3*d})`,a.fill()))}s.current=requestAnimationFrame(c)};return s.current=requestAnimationFrame(c),window.addEventListener("resize",l),()=>{cancelAnimationFrame(s.current),window.removeEventListener("resize",l)}},[r]),(0,t.jsxs)("div",{className:"pointer-events-none absolute inset-0 overflow-hidden",children:[(0,t.jsx)("div",{className:"absolute inset-0",style:{backgroundImage:"linear-gradient(rgba(255,255,255,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.03) 1px, transparent 1px)",backgroundSize:"64px 64px"}}),(0,t.jsx)("canvas",{ref:e,className:"absolute inset-0 w-full h-full",style:{opacity:.55}}),(0,t.jsx)("div",{className:"absolute inset-0",style:{background:"radial-gradient(ellipse 80% 60% at 50% 40%, transparent 0%, rgba(0,0,0,0.65) 100%)"}})]})}var p=e.i(63060),g=e.i(25);function f({text:e,font:s='300 18px "Geist"',lineHeight:o=30,className:r,style:n,fallback:i,cursorRadius:l=36,cursorColor:c="var(--accent)"}){let h=(0,a.useRef)(null),{ready:d,prepareWithSegments:u,layoutNextLine:m}=(0,g.usePreText)("Geist");return((0,a.useEffect)(()=>{if(!d||!u||!m||!h.current)return;let t=h.current,a=u(e,s);if(!a)return;let r=!1,n=0,i=0,p=0,g=0;for(;t.firstChild;)t.removeChild(t.firstChild);let f=[],y=document.createElement("div");function w(e){let s=t.clientWidth;if(0===s)return[];let r={segmentIndex:0,graphemeIndex:0},n=0,i=[],l=0;for(;l++<500;){let t=s,l=0;if(e&&n+o>e.y&&n<e.y+e.height){let a=e.x,o=e.x+e.width;if(a<=0)t=s-(l=Math.max(0,o+12));else if(o>=s)t=Math.max(60,a-12);else{let e=a-12,r=s-o-12;e>=r?t=Math.max(60,e):(l=o+12,t=Math.max(60,r))}}t=Math.max(t,60);let c=r.segmentIndex,h=r.graphemeIndex,d=m(a,r,t);if(!d||(i.push({text:d.text.trimEnd(),y:n,indent:l}),r=d.end,n+=o,r.segmentIndex===c&&r.graphemeIndex===h))break}return i}function b(e){for(var a=e.length;f.length<a;){let e=document.createElement("div");e.style.cssText=`
