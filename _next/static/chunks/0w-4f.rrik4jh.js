(globalThis.TURBOPACK||(globalThis.TURBOPACK=[])).push(["object"==typeof document?document.currentScript:void 0,43875,e=>{"use strict";var t=e.i(26869),a=e.i(14363),s=e.i(87212);let o=(0,e.i(72592).default)("arrow-right",[["path",{d:"M5 12h14",key:"1ays0h"}],["path",{d:"m12 5 7 7-7 7",key:"xquz4c"}]]);var n=e.i(81334),i=e.i(91195);function r(e,t){i.default.capture("cta_clicked",{cta_name:e,destination:t})}function l({text:e,className:a,delay:s=0}){let o=e.split(" ");return(0,t.jsxs)("span",{className:a,children:[o.map((e,a)=>(0,t.jsx)("span",{className:"inline-block mr-[0.25em]",style:{animation:`fadeInUp 0.5s cubic-bezier(0.16,1,0.3,1) ${s+.08*a}s both`},children:e},a)),(0,t.jsx)("style",{children:`
        @keyframes fadeInUp {
          from { opacity: 0; transform: translateY(20px); filter: blur(8px); }
          to { opacity: 1; transform: translateY(0); filter: blur(0); }
        }
      `})]})}function h({children:e,className:a,delay:s=0}){return(0,t.jsxs)("div",{className:a,style:{animation:`fadeInUpSimple 0.6s cubic-bezier(0.16,1,0.3,1) ${s}s both`},children:[e,(0,t.jsx)("style",{children:`
        @keyframes fadeInUpSimple {
          from { opacity: 0; transform: translateY(16px); }
          to { opacity: 1; transform: translateY(0); }
        }
      `})]})}function c({children:e,className:a="",href:o,glowColor:n="var(--accent)"}){let i=(0,t.jsxs)("div",{className:`group relative overflow-hidden border p-8 transition-all duration-200 hover:scale-[1.01] texture-dots ${a}`,style:{background:"var(--bg-surface)",borderColor:"var(--border-subtle)"},children:[(0,t.jsx)("div",{className:"pointer-events-none absolute -inset-px opacity-0 transition-opacity duration-300 group-hover:opacity-100",style:{background:`radial-gradient(600px circle at var(--mouse-x, 50%) var(--mouse-y, 50%), ${n}10, transparent 40%)`}}),(0,t.jsx)("div",{className:"pointer-events-none absolute inset-x-0 top-0 h-px opacity-0 transition-opacity duration-300 group-hover:opacity-100",style:{background:`linear-gradient(90deg, transparent, ${n}30, transparent)`}}),(0,t.jsx)("div",{className:"relative",children:e})]});return o?o.startsWith("/")?(0,t.jsx)(s.default,{href:o,children:i}):(0,t.jsx)("a",{href:o,target:"_blank",rel:"noopener noreferrer",children:i}):i}let d=["lorenz","rossler","spiral","flow"],u=[[235,85],[160,70],[270,75],[210,80],[330,65],[180,60]];function p(){let e=(0,a.useRef)(null),s=(0,a.useRef)(0),o=(0,a.useRef)(null),n=(0,a.useCallback)(e=>{var t;let a=(Date.now()^0xffffffff*Math.random())>>>0,s=(t=a,()=>{t|=0;let e=Math.imul((t=t+0x6d2b79f5|0)^t>>>15,1|t);return(((e=e+Math.imul(e^e>>>7,61|e)^e)^e>>>14)>>>0)/0x100000000}),n=d[Math.floor(s()*d.length)],i=u[Math.floor(s()*u.length)],r=e.width,l=e.height;o.current={particles:Array.from({length:600},()=>{let e=s()*r,t=s()*l;return{x:e,y:t,prevX:e,prevY:t,vx:(s()-.5)*2,vy:(s()-.5)*2,life:Math.floor(250*s())+80,maxLife:330,hueOffset:(s()-.5)*40}}),type:n,seed:a,rng:s,cx1:r*(.2+.6*s()),cy1:l*(.2+.6*s()),cx2:r*(.2+.6*s()),cy2:l*(.2+.6*s()),cx3:r*(.2+.6*s()),cy3:l*(.2+.6*s()),strength:.012+.025*s(),baseHue:i[0]+(s()-.5)*30,baseSat:i[1],hueRange:20+40*s(),trailFade:.03+.03*s(),time:0}},[]);return(0,a.useEffect)(()=>{let t=e.current;if(!t)return;let a=t.getContext("2d");if(!a)return;let i=0,r=0,l=()=>{let e=Math.min(window.devicePixelRatio||1,2),s=t.getBoundingClientRect();i=s.width,r=s.height,t.width=s.width*e,t.height=s.height*e,a.scale(e,e),n(t)};l();let h=()=>{let e=o.current;if(!e)return;e.time++,a.fillStyle=`rgba(0, 0, 0, ${e.trailFade})`,a.fillRect(0,0,i,r);let n=i/t.width,l=r/t.height;for(let s of e.particles){!function(e,t,a,s){e.prevX=e.x,e.prevY=e.y;let o=0,n=0,{type:i,cx1:r,cy1:l,cx2:h,cy2:c,cx3:d,cy3:u,strength:p,time:m}=t;switch(i){case"lorenz":{let t=r-e.x,a=l-e.y,s=h-e.x,i=c-e.y,m=Math.sqrt(t*t+a*a)+1,g=Math.sqrt(s*s+i*i)+1,f=d-e.x,y=u-e.y,w=Math.sqrt(f*f+y*y)+1;o=(t/m-i/g+f/w*.3)*p,n=(a/m+s/g-y/w*.3)*p;break}case"rossler":{let t=r-e.x,a=l-e.y,s=Math.sqrt(t*t+a*a)+1,i=.005*Math.sin(.002*m);o=(-a/s+t*(.008+i))*p,n=(t/s+a*(.008+i))*p;let d=h-e.x,u=c-e.y,g=Math.sqrt(d*d+u*u)+1;o+=u/g*p*.2,n-=d/g*p*.2;break}case"spiral":{let t=r-e.x,a=l-e.y,s=Math.sqrt(t*t+a*a)+1,i=Math.atan2(a,t)+.3+.2*Math.sin(.001*m);o=Math.cos(i)/s*p*8,n=Math.sin(i)/s*p*8;let d=h-e.x,u=c-e.y,g=Math.sqrt(d*d+u*u)+1,f=Math.atan2(u,d)-.5;o+=Math.cos(f)/g*p*3,n+=Math.sin(f)/g*p*3;break}case"flow":{let t=.002+5e-4*Math.sin(5e-4*m),i=Math.sin(e.x*t+.001*m)*Math.cos(e.y*t*.7),r=Math.cos(e.x*t*.8-8e-4*m)*Math.sin(e.y*t);o=i*p*1.5,n=r*p*1.5,o+=(.5*a-e.x)*1e-5,n+=(.5*s-e.y)*1e-5}}e.vx+=o,e.vy+=n,e.vx*=.982,e.vy*=.982,e.x+=e.vx,e.y+=e.vy,e.life--,e.life<=0&&(e.x=t.rng()*a,e.y=t.rng()*s,e.prevX=e.x,e.prevY=e.y,e.vx=(t.rng()-.5)*2,e.vy=(t.rng()-.5)*2,e.life=Math.floor(250*t.rng())+80,e.hueOffset=(t.rng()-.5)*40),e.x<-20&&(e.x=-20,e.vx*=-.5),e.x>a+20&&(e.x=a+20,e.vx*=-.5),e.y<-20&&(e.y=-20,e.vy*=-.5),e.y>s+20&&(e.y=s+20,e.vy*=-.5)}(s,e,t.width,t.height);let o=s.life/s.maxLife,h=o>.9?(1-o)*10:o<.2?5*o:1,c=Math.sqrt(s.vx*s.vx+s.vy*s.vy),d=h*Math.min(.8,.2+.15*c),u=.5+.4*c,p=e.baseHue+s.hueOffset+Math.sin(.003*e.time+s.hueOffset)*e.hueRange*.3,m=s.prevX*n,g=s.prevY*l,f=s.x*n,y=s.y*l,w=Math.abs(y-g);Math.abs(f-m)>.3*i||w>.3*r||(a.beginPath(),a.moveTo(m,g),a.lineTo(f,y),a.strokeStyle=`hsla(${p}, ${e.baseSat}%, 65%, ${d})`,a.lineWidth=u,a.lineCap="round",a.stroke(),c>2.5&&(a.beginPath(),a.arc(f,y,u+2,0,2*Math.PI),a.fillStyle=`hsla(${p}, ${e.baseSat}%, 70%, ${.3*d})`,a.fill()))}s.current=requestAnimationFrame(h)};return s.current=requestAnimationFrame(h),window.addEventListener("resize",l),()=>{cancelAnimationFrame(s.current),window.removeEventListener("resize",l)}},[n]),(0,t.jsxs)("div",{className:"pointer-events-none absolute inset-0 overflow-hidden",children:[(0,t.jsx)("div",{className:"absolute inset-0",style:{backgroundImage:"linear-gradient(rgba(255,255,255,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.03) 1px, transparent 1px)",backgroundSize:"64px 64px"}}),(0,t.jsx)("canvas",{ref:e,className:"absolute inset-0 w-full h-full",style:{opacity:.55}}),(0,t.jsx)("div",{className:"absolute inset-0",style:{background:"radial-gradient(ellipse 80% 60% at 50% 40%, transparent 0%, rgba(0,0,0,0.65) 100%)"}})]})}var m=e.i(63060),g=e.i(25);function f({text:e,font:s='300 18px "Geist"',lineHeight:o=30,className:n,style:i,fallback:r,cursorRadius:l=36,cursorColor:h="var(--accent)"}){let c=(0,a.useRef)(null),{ready:d,prepareWithSegments:u,layoutNextLine:p}=(0,g.usePreText)("Geist");return((0,a.useEffect)(()=>{if(!d||!u||!p||!c.current)return;let t=c.current,a=u(e,s);if(!a)return;let n=!1,i=0,r=0,m=0,g=0;for(;t.firstChild;)t.removeChild(t.firstChild);let f=[],y=document.createElement("div");function w(e){let s=t.clientWidth;if(0===s)return[];let n={segmentIndex:0,graphemeIndex:0},i=0,r=[],l=0;for(;l++<500;){let t=s,l=0;if(e&&i+o>e.y&&i<e.y+e.height){let a=e.x,o=e.x+e.width;if(a<=0)t=s-(l=Math.max(0,o+12));else if(o>=s)t=Math.max(60,a-12);else{let e=a-12,n=s-o-12;e>=n?t=Math.max(60,e):(l=o+12,t=Math.max(60,n))}}t=Math.max(t,60);let h=n.segmentIndex,c=n.graphemeIndex,d=p(a,n,t);if(!d||(r.push({text:d.text.trimEnd(),y:i,indent:l}),n=d.end,i+=o,n.segmentIndex===h&&n.graphemeIndex===c))break}return r}function b(e){for(var a=e.length;f.length<a;){let e=document.createElement("div");e.style.cssText=`
          position: absolute; white-space: pre; pointer-events: none;
          font: ${s}; line-height: ${o}px; height: ${o}px;
          left: 0; top: 0;
        `,t.appendChild(e),f.push(e)}for(let e=a;e<f.length;e++)f[e].style.display="none";for(let t=0;t<e.length;t++){let a=f[t],s=e[t];a.style.display="block",a.textContent!==s.text&&(a.textContent=s.text),a.style.transform=`translate(${s.indent}px, ${s.y}px)`,a.style.opacity="1"}let n=e.length*o;n>0&&(t.style.height=`${Math.max(n,g)}px`)}y.style.cssText=`
      position: absolute; border-radius: 50%; pointer-events: none;
      width: ${2*l}px; height: ${2*l}px;
      background: ${h}; filter: blur(16px);
      mix-blend-mode: screen; opacity: 0;
      transition: opacity 0.25s ease;
      z-index: 1;
    `,t.appendChild(y);let v=w(null);g=v.length*o,b(v);for(let e=0;e<f.length;e++)f[e].style.opacity="0",f[e].style.transition="opacity 0.4s ease, transform 0.35s cubic-bezier(0.16, 1, 0.3, 1)";function k(){n=!0,y.style.opacity="0.15",m||A()}function T(e){let a=t.getBoundingClientRect();i=e.clientX-a.left,r=e.clientY-a.top,y.style.transform=`translate(${i-l}px, ${r-l}px)`,n||(n=!0,y.style.opacity="0.15",m||A())}function x(){n=!1,y.style.opacity="0",setTimeout(()=>{n||b(w(null))},300)}function A(){if(!n){m=0;return}m=requestAnimationFrame(A),b(w({x:i-l-14,y:r-l-14,width:(l+14)*2,height:(l+14)*2}))}return requestAnimationFrame(()=>{for(let e=0;e<v.length;e++)setTimeout(()=>{f[e]&&(f[e].style.opacity="1")},80*e)}),t.style.cursor="crosshair",t.addEventListener("pointerenter",k),t.addEventListener("pointermove",T),t.addEventListener("pointerleave",x),()=>{cancelAnimationFrame(m),t.removeEventListener("pointerenter",k),t.removeEventListener("pointermove",T),t.removeEventListener("pointerleave",x)}},[d,e,s,o,u,p,l,h]),d)?(0,t.jsx)("div",{ref:c,className:n,style:{...i,position:"relative",overflow:"visible"}}):(0,t.jsx)("div",{ref:c,className:n,style:i,children:r??e})}let y={segmentIndex:0,graphemeIndex:0};function w({segments:e,className:s,style:o}){let n=(0,a.useRef)(null),[i,r]=(0,a.useState)(0),{ready:l,prepareWithSegments:h,layoutNextLine:c}=(0,g.usePreText)("Geist");(0,a.useEffect)(()=>{if(!n.current)return;let e=()=>{n.current&&r(n.current.clientWidth)};e();let t=new ResizeObserver(e);return t.observe(n.current),()=>t.disconnect()},[]);let d=(0,a.useMemo)(()=>{if(!l||!h||!c||i<=0)return null;let t=e.map(e=>({prepared:h(e.text,e.font),segment:e}));if(t.some(e=>!e.prepared))return null;let a=[],s=Math.max(...e.map(e=>e.lineHeight)),o=0,n=[],r=i;for(let{prepared:e,segment:l}of t){if(!e)continue;let t=c(e,y,1e5);if(t)if(t.width<=r)n.push({text:t.text,width:t.width,style:l.style,font:l.font}),r-=t.width;else{let t=y;for(;;){let h=c(e,t,Math.max(1,r));if(!h)break;if(n.push({text:h.text,width:h.width,style:l.style,font:l.font}),h.end.segmentIndex>=e.segments.length||h.end.segmentIndex===e.segments.length-1&&h.end.graphemeIndex>=e.segments[e.segments.length-1].length){r-=h.width;break}a.push({fragments:n,y:o,lineHeight:s}),o+=s,n=[],r=i,t=h.end}}}return n.length>0&&(a.push({fragments:n,y:o,lineHeight:s}),o+=s),{lines:a,totalHeight:o}},[l,h,c,e,i]);return d?(0,t.jsx)("div",{ref:n,className:s,style:{...o,position:"relative",height:`${d.totalHeight}px`},children:d.lines.map((e,a)=>(0,t.jsx)("div",{style:{position:"absolute",top:`${e.y}px`,left:0,height:`${e.lineHeight}px`,lineHeight:`${e.lineHeight}px`,whiteSpace:"pre"},children:e.fragments.map((e,a)=>(0,t.jsx)("span",{style:{...e.style,font:e.font},children:e.text},a))},a))}):(0,t.jsx)("div",{ref:n,className:s,style:{...o,whiteSpace:"pre-wrap"},children:e.map((e,a)=>(0,t.jsx)("span",{style:{...e.style,font:e.font},children:e.text},a))})}function b({text:e,font:s='300 14px "Geist"',lineHeight:o=24,orbCount:n=3,orbRadius:i=40,className:r,style:l,textColor:h="var(--text-secondary)"}){let{ready:c,prepareWithSegments:d,layoutNextLine:u}=(0,g.usePreText)("Geist"),p=(0,a.useRef)(null),m=(0,a.useRef)(null),f=(0,a.useRef)(null),y=(0,a.useRef)([]),w=(0,a.useRef)(0),[v,k]=(0,a.useState)({width:0,height:0}),T=(0,a.useRef)(-1),x=(0,a.useRef)({x:0,y:0});(0,a.useEffect)(()=>{if(0===v.width)return;let e=v.width,t=v.height||300,a=["rgba(129,140,248,0.12)","rgba(34,197,94,0.10)","rgba(129,140,248,0.08)"];y.current=Array.from({length:n},(s,o)=>({x:e*(.25+.5*Math.random()),y:t*(.25+.5*Math.random()),vx:(Math.random()-.5)*1.5,vy:(Math.random()-.5)*1.5,radius:i+15*Math.random(),color:a[o%a.length],dragging:!1}))},[v.width,v.height,n,i]),(0,a.useEffect)(()=>{if(!p.current)return;let e=()=>{if(p.current){let{width:e}=p.current.getBoundingClientRect();k({width:e,height:280})}};e();let t=new ResizeObserver(e);return t.observe(p.current),()=>t.disconnect()},[c]),(0,a.useEffect)(()=>{if(!c||!d||!u||0===v.width)return;let t=d(e,s);if(!t)return;let a=m.current,n=f.current;if(!a||!n)return;let i=a.getContext("2d");if(!i)return;let r=v.width,l=window.devicePixelRatio||1,p=()=>{let e=y.current;for(let t of e)!t.dragging&&(t.x+=t.vx,t.y+=t.vy,t.x-t.radius<0&&(t.x=t.radius,t.vx*=-.8),t.x+t.radius>r&&(t.x=r-t.radius,t.vx*=-.8),t.y-t.radius<0&&(t.y=t.radius,t.vy*=-.8),t.vx*=.999,t.vy*=.999,.3>Math.sqrt(t.vx*t.vx+t.vy*t.vy)&&(t.vx+=(Math.random()-.5)*.2,t.vy+=(Math.random()-.5)*.2));for(let t=0;t<e.length;t++)for(let a=t+1;a<e.length;a++){let s=e[t],o=e[a],n=o.x-s.x,i=o.y-s.y,r=Math.sqrt(n*n+i*i),l=s.radius+o.radius;if(r<l&&r>0){let e=n/r,t=i/r,a=(l-r)/2;s.dragging||(s.x-=e*a,s.y-=t*a),o.dragging||(o.x+=e*a,o.y+=t*a);let h=(o.vx-s.vx)*e+(o.vy-s.vy)*t;s.dragging||(s.vx+=h*e*.5,s.vy+=h*t*.5),o.dragging||(o.vx-=h*e*.5,o.vy-=h*t*.5)}}let c=[],d={segmentIndex:0,graphemeIndex:0},m=0,g=t.segments.length,f=200;for(;(d.segmentIndex<g||d.graphemeIndex>0)&&m<280&&f-- >0;){let a=0,s=r;for(let t of e){let e=t.y-t.radius,n=t.y+t.radius;if(m+o>e&&m<n){let e=m+o/2-t.y,n=Math.sqrt(Math.max(0,t.radius*t.radius-e*e)),i=t.x-n-8,l=t.x+n+8,h=Math.max(0,i),c=Math.max(0,r-l);if(h>=c&&h>60)s=h,a=0;else if(c>60)a=l,s=c;else{m+=o;continue}}}if(s<40){m+=o;continue}let n=u(t,d,s);if(!n||n.end.segmentIndex===d.segmentIndex&&n.end.graphemeIndex===d.graphemeIndex)break;n.text.trim()&&c.push({x:a,y:m,text:n.text}),d=n.end,m+=o}for(let t of(a.width=r*l,a.height=280*l,a.style.width=`${r}px`,a.style.height="280px",i.scale(l,l),i.clearRect(0,0,r,280),e)){let e=i.createRadialGradient(t.x,t.y,0,t.x,t.y,t.radius);e.addColorStop(0,t.color.replace(/[\d.]+\)$/,"0.15)")),e.addColorStop(.7,t.color),e.addColorStop(1,"transparent"),i.beginPath(),i.arc(t.x,t.y,t.radius,0,2*Math.PI),i.fillStyle=e,i.fill(),i.beginPath(),i.arc(t.x,t.y,t.radius,0,2*Math.PI),i.strokeStyle=t.color.replace(/[\d.]+\)$/,"0.3)"),i.lineWidth=1,i.stroke()}for(let e of(n.innerHTML="",n.style.height="280px",c)){let t=document.createElement("div");t.style.position="absolute",t.style.left=`${e.x}px`,t.style.top=`${e.y}px`,t.style.font=s,t.style.color=h,t.style.whiteSpace="nowrap",t.style.lineHeight=`${o}px`,t.textContent=e.text,n.appendChild(t)}for(let t of e)t.y+t.radius>270&&(t.y=280-t.radius-10,t.vy*=-.8);w.current=requestAnimationFrame(p)};return w.current=requestAnimationFrame(p),()=>cancelAnimationFrame(w.current)},[c,d,u,v.width,e,s,o,h]);let A=(0,a.useCallback)(e=>{let t=p.current?.getBoundingClientRect();if(!t)return;let a=e.clientX-t.left,s=e.clientY-t.top;for(let t=0;t<y.current.length;t++){let o=y.current[t],n=a-o.x,i=s-o.y;if(n*n+i*i<o.radius*o.radius){T.current=t,x.current={x:n,y:i},o.dragging=!0,o.vx=0,o.vy=0,e.target.setPointerCapture(e.pointerId);return}}},[]),I=(0,a.useCallback)(e=>{if(T.current<0)return;let t=p.current?.getBoundingClientRect();if(!t)return;let a=y.current[T.current];a&&(a.x=e.clientX-t.left-x.current.x,a.y=e.clientY-t.top-x.current.y)},[]),C=(0,a.useCallback)(()=>{if(T.current>=0){let e=y.current[T.current];e&&(e.dragging=!1)}T.current=-1},[]);return c?(0,t.jsxs)("div",{ref:p,className:r,style:{...l,position:"relative",height:280,overflow:"hidden",cursor:"default"},onPointerDown:A,onPointerMove:I,onPointerUp:C,children:[(0,t.jsx)("canvas",{ref:m,style:{position:"absolute",top:0,left:0,zIndex:0,pointerEvents:"none"}}),(0,t.jsx)("div",{ref:f,style:{position:"absolute",top:0,left:0,width:"100%",zIndex:1,pointerEvents:"none"}})]}):(0,t.jsx)("div",{className:r,style:l,children:(0,t.jsx)("p",{style:{color:h,font:s,lineHeight:`${o}px`},children:e})})}var v=e.i(81154);let k="utm_source=edgelesslab&utm_medium=website&utm_campaign=products",T=[{name:"CLAUDE.md Template Pack",price:"Free",description:"14 battle-tested CLAUDE.md templates for every project type. Drop one into your repo and start building.",features:["14 templates: iOS, Android, ML, API, DevOps, Next.js, and more","CLI Tools, Monorepos, and Game Dev configurations","Embedded/IoT, Security Audits, and Open Source templates","Startup MVP and Technical Writing presets"],href:`https://edgelessai.gumroad.com/l/kszapk?${k}`,badge:"Free"},{name:"Quick Reference Cards",price:"Free",description:"Printable cheat sheets for prompt patterns, Claude Code shortcuts, MCP tool reference, and common workflows.",features:["Prompt pattern and system prompt reference cards","Claude Code shortcuts and slash commands","Token optimization and temperature settings guide","PDF and markdown formats, pin-next-to-monitor ready"],href:`https://edgelessai.gumroad.com/l/dihxts?${k}`,badge:"Free"},{name:"Claude Code Cheat Sheet",price:"Free",description:"Quick-start reference for Claude Code. Commands, shortcuts, hook patterns, and MCP setup in one printable sheet.",features:["All slash commands and keyboard shortcuts","Hook configuration patterns with examples","MCP server setup cheatsheet","CLAUDE.md template and memory system quick-start"],href:`https://edgelessai.gumroad.com/l/claude-code-cheat-sheet?${k}`,badge:"Free",slug:"claude-code-cheat-sheet"},{name:"Claude Memory Kit",price:"Free",description:"Start here. Drop-in memory template for Claude Code. Persists context, feedback, and project knowledge across conversations.",features:["4 memory types: user, feedback, project, reference","MEMORY.md index auto-loaded each session","CLAUDE.md snippet for instant setup","Real-world examples included"],href:"https://github.com/edgeless-ai/claude-memory-kit?utm_source=edgelesslab&utm_medium=website&utm_campaign=products",badge:"Free",repoUrl:"https://github.com/edgeless-ai/claude-memory-kit"},{name:"Edgeless Stack",price:"Free",description:"The complete AI agent infrastructure. Memory that persists, hooks that protect, skills that compound, and agents that run while you sleep. Extracted from 3+ months of production use.",features:["3-layer memory: SQLite ledger + ChromaDB vectors + Obsidian vault","6 safety hooks: damage control, completion verification, taxonomy guard","9 skills with tiered loading (5 core + 4 domain)","Cron patterns, Agent Bus, Docker compose, install.sh"],href:"https://github.com/edgeless-ai/edgeless-stack?utm_source=edgelesslab&utm_medium=website&utm_campaign=products",badge:"Free",repoUrl:"https://github.com/edgeless-ai/edgeless-stack"},{name:"Multi-Agent Orchestration Blueprint",price:"Free",description:"The dispatch/worker architecture for coordinating multiple AI agents. Agent Bus messaging, async inboxes, state machines, and 3 reference implementations from a system that runs 5 agents 24/7.",features:["Dispatch/worker topology: routing tasks to specialist agents","Agent Bus setup: real-time inter-session messaging patterns","State machines: queued -> acked -> running -> done/failed","3 reference pipelines: research, code review, content processing"],href:`https://edgelessai.gumroad.com/l/multi-agent-blueprint?${k}`,badge:"New",comingSoon:!1,slug:"multi-agent-blueprint"},{name:"The Agent Cookbook",price:"Free",description:"Build AI agents that actually work. 15 production-ready agent patterns with complete implementations for Claude, GPT, and open-source models.",features:["15 production-ready agent patterns with working code","Memory systems, tool integration, and context management","Error recovery and deployment strategies","Architecture diagrams and production lessons learned"],href:`https://edgelessai.gumroad.com/l/plbzo?${k}`,badge:null},{name:"Claude Memory Kit Pro",price:"Free",description:"The complete memory system for Claude Code power users. 12 templates, 5 stack libraries, advanced patterns guide, and CLAUDE.md templates.",features:["12 ready-to-customize memory templates","Stack libraries: React/Next.js, Python/FastAPI, Go, Rails, Rust","Advanced patterns: multi-project, team memory, CI integration","CLAUDE.md templates for solo and monorepo projects"],href:`https://edgelessai.gumroad.com/l/claude-memory-kit?${k}`,badge:"Popular"},{name:"The Prompt Engineering OS",price:"Free",description:"The complete system for writing AI prompts that work in production. 30 chapters, 8 template schemas, 100+ templates.",features:["30 chapters covering every prompt pattern","8 template schemas with fill-in-the-blank structure","100+ production-tested prompt templates","Covers Claude, GPT-4, Gemini, and open models"],href:`https://edgelessai.gumroad.com/l/prompt-engineering-os?${k}`,badge:null},{name:"Generative Art Starter Kit",price:"Free",description:"10 Python generators for pen plotters: flow fields, L-systems, Voronoi, spirals, reaction-diffusion. Each with parameter guides, example SVGs, and AI scoring rubrics from 105+ experiments.",features:["10 generators with source code, parameter guides, and 3 example outputs each","SVG optimization for pen plotters: stroke ordering, travel minimization","AI scoring rubric for evaluating generative art quality","Print-ready export: A4, A3, letter sizes with plotter setup guides"],href:`https://edgelessai.gumroad.com/l/gen-art-starter?${k}`,badge:"New",comingSoon:!1,slug:"gen-art-starter"},{name:"Always-On Agent Deployment Kit",price:"Free",description:"Deploy an AI agent that runs 24/7 on a $5 VPS. Cron scheduling, grounding packets, memory contracts, Telegram alerting, and the recovery patterns from 3 months of unattended operation.",features:["VPS setup guide: Hetzner, systemd, PM2, environment hardening","Cron job architecture: health checks, email triage, knowledge consolidation","Grounding stack: verification layers, evidence-based completion, session packets","Communication patterns: Telegram bot, inbox dispatch, API access"],href:`https://edgelessai.gumroad.com/l/always-on-agent?${k}`,badge:"New",comingSoon:!0,slug:"always-on-agent"},{name:"Production MCP Server Kit",price:"Free",description:"Take MCP servers past the tutorial stage. Auth middleware, rate limiting, Docker deployment, health checks, and error handling patterns from running 4+ MCP servers in production.",features:["Auth middleware: API key validation and OAuth2 token checking","Rate limiting, usage tracking, and health check endpoints","Docker + compose deployment configs with monitoring","3 production server examples: filesystem, database, external API"],href:`https://edgelessai.gumroad.com/l/production-mcp-kit?${k}`,badge:"New",comingSoon:!1,slug:"production-mcp-kit"},{name:"AI Code Review Playbook",price:"Free",description:"Systematic AI-powered code review that catches security vulnerabilities, performance issues, and logic errors before they ship.",features:["Review checklists and prompt templates for Claude/GPT","GitHub Actions and CI/CD integration guides","Security vulnerability and performance issue detection","Built from real experience reviewing thousands of PRs"],href:`https://edgelessai.gumroad.com/l/uacjr?${k}`,badge:null},{name:"Digital Product Launch Toolkit",price:"Free",description:"The exact process used to ship 18 digital products as a solo developer. Gumroad page templates, pricing strategy, launch checklists, and the daily shipping workflow.",features:["3 Gumroad page layouts: simple, detailed, and premium tiers","Pricing strategy guide: why $9/$14/$19/$24/$29/$39 tiers work","18-step launch checklist from idea to live listing","Cross-sell and bundle strategies with real revenue examples"],href:`https://edgelessai.gumroad.com/l/launch-toolkit?${k}`,badge:"New",comingSoon:!1,slug:"launch-toolkit"},{name:"n8n AI Workflow Templates",price:"Free",description:"5 importable n8n workflows that connect AI to real business processes. YouTube monitoring, RSS intelligence, AI code review, content embedding, and scheduled health checks.",features:["5 ready-to-import n8n workflow JSON files","YouTube monitor -> Claude summary -> email digest pipeline","RSS aggregator -> AI analysis -> Telegram notification","Docker n8n setup guide with environment configuration"],href:`https://edgelessai.gumroad.com/l/n8n-ai-workflows?${k}`,badge:"New",comingSoon:!1,slug:"n8n-ai-workflows"},{name:"MCP Server Starter Kit",price:"Free",description:"TypeScript and Python templates for building MCP servers. Go from zero to a running server in under an hour.",features:["TypeScript and Python server templates","8-chapter guide from architecture to deployment","3 working example servers: file search, database query, API proxy","Complete build-to-deploy walkthrough"],href:`https://edgelessai.gumroad.com/l/lixicg?${k}`,badge:null},{name:"Obsidian + Claude Code Setup Kit",price:"Free",description:"Turn Obsidian into an AI-powered development environment with pre-configured vault, Claude Code integration, and workflow automations.",features:["Pre-configured vault with Claude Code integration","CLAUDE.md templates and hook configurations","Custom templates and plugin recommendations","Complete knowledge management system for AI developers"],href:`https://edgelessai.gumroad.com/l/fyuwpn?${k}`,badge:null},{name:"Prompt Testing Framework",price:"Free",description:"Regression testing, A/B comparison templates, and quality scoring rubrics for AI prompts. Built for teams shipping AI features.",features:["Regression testing and A/B comparison templates","Quality scoring rubrics with structured evaluation criteria","Test harnesses for Claude, GPT, and Gemini","Repeatable, measurable prompt quality workflows"],href:`https://edgelessai.gumroad.com/l/yrail?${k}`,badge:null},{name:"Autonomous Agent Safety Patterns",price:"Free",description:"Hard-won guardrails from an agent that lost $252 of real money. Financial verification protocols, destructive operation prevention, scope containment, and the incident response playbook.",features:["Full post-mortem: the $252 USDC loss and what changed after","10 anti-patterns with production fixes and hook implementations","Financial transaction verification protocol (test small, verify, confirm)","Scope containment patterns: keeping agents within boundaries"],href:`https://edgelessai.gumroad.com/l/agent-safety-patterns?${k}`,badge:"New",comingSoon:!1,slug:"agent-safety-patterns"},{name:"Claude Code Hooks Deep Dive",price:"Free",description:"15 production hooks beyond the basics. The damage-control hook that blocks destructive commands. The verify-completion hook that won't let you lie about finishing. Session init, memory flush, pre-commit guards.",features:["15 battle-tested hooks with full source and walkthroughs","damage-control.py: the hook that saved the codebase from rm -rf","Hook composition patterns: chaining, conditional, env-aware","Template hooks for common scenarios you can customize in minutes"],href:`https://edgelessai.gumroad.com/l/hooks-deep-dive?${k}`,badge:"New",comingSoon:!1,slug:"hooks-deep-dive"},{name:"Hooks Library",price:"Free",description:"24 production-ready hooks across 6 categories. Drop in, configure, ship.",features:["Quality hooks: linting, testing, secrets detection","Safety hooks: damage control, backup, force-push guard","Integration hooks: Slack, Telegram, Linear, Obsidian","AI hooks: context preload, completion verify, cost tracking"],href:`https://edgelessai.gumroad.com/l/ztaflt?${k}`,badge:null}];v.PLOTTER.pieces,v.PLOTTER.factories,v.PLOTTER.factories,v.PLOTTER.factories,v.PLOTTER.pieces,v.PLOTTER.factories,v.PLOTTER.pieces,v.PLOTTER.best,v.PLOTTER.factories,v.PLOTTER.pieces,v.PLOTTER.kept,v.PLOTTER.kept,v.PLOTTER.factories,v.PLOTTER.pieces,v.PLOTTER.best,v.PLOTTER.kept;let x=[{slug:"flow-viz-bitcoin-mempool",title:"Flow Viz: Watching the Bitcoin Mempool Breathe",description:"Live transaction data becomes fluid particle simulation. Flow Viz turns the Bitcoin fee market into something you can watch with your eyes.",date:"2026-05-20",tags:["Field Notes","Creative Coding","Data Visualization","Bitcoin","p5.js"],readTime:"6 min",editorial:!0,content:`
...
*Edgeless Lab builds infrastructure for autonomous AI systems. And sometimes, tools for watching them.*
    `.trim()},{slug:"envelope-protocol-multi-agent-coordination",title:"We Built an Envelope Protocol to Stop Our AI Agents from Talking Forever",description:"Seven Discord bots needed to coordinate without infinite loops. A five-field envelope header and a depth counter solved it. Here's the protocol.",date:"2026-05-13",tags:["AI Agents","Multi-Agent Systems","Discord","Protocol Design"],readTime:"8 min",editorial:!0,content:`
...
*Edgeless Lab builds infrastructure for autonomous AI systems.*
    `.trim()},{slug:"generative-art-pen-plotter-pipeline",title:"From Algorithm to Ink: How We Turn Generative Code into Physical Art",description:"65 algorithms, a texture pipeline, and a pen plotter. The toolchain that turns SVG code into framed prints on the wall.",date:"2026-05-13",tags:["Generative Art","Pen Plotting","Creative Coding","Design System"],readTime:"8 min",editorial:!0,content:`
...
*Edgeless Lab builds infrastructure for autonomous AI systems. And occasionally, art.*
    `.trim()},{slug:"spend-breaker-circuit-breaker-llm-costs",title:"A Circuit Breaker Saved Me $200 in Tokens. It Took 45 Minutes to Build.",description:"One of my AI agents consumed 60% of its daily token budget in four hours. The circuit breaker throttled it automatically. Here's the pattern.",date:"2026-05-13",tags:["AI Agents","Cost Management","Circuit Breaker","Infrastructure"],readTime:"7 min",editorial:!0,content:`
...
*Edgeless Lab builds infrastructure for autonomous AI systems.*
    `.trim()},{slug:"agents-that-improve-themselves",title:"The Most Useful Thing Your AI Agents Can Do Is Audit Themselves",description:"We pointed our agents at our own knowledge base and asked what they should be doing that they weren't. Then we built the fixes.",date:"2026-05-12",tags:["AI Agents","Self-Improvement","Automation","Knowledge Management"],readTime:"7 min",editorial:!0,content:`
...
*Edgeless Lab builds infrastructure for autonomous AI systems.*
    `.trim()},{slug:"youtube-mining-ai-agents",title:"I Pointed 7 AI Agents at My YouTube History. They Found What I Couldn't See.",description:"7 agents analyzed 1,062 YouTube videos and found 14 things they should be doing that they weren't. Topic clusters, channel ROI, knowledge gaps, and the meta-move: agents auditing themselves.",date:"2026-05-12",tags:["AI Agents","YouTube","Knowledge Mining","Automation"],readTime:"10 min",editorial:!0,content:`
...
*David runs Edgeless Lab, a solo creative technology practice. He has 25 AI agents, 168 skills, and 1,062 YouTube vault notes. The agents now audit their own knowledge gaps. He's not sure who's running the operation anymore.*
    `.trim()},{slug:"real-cost-ai-agents-production-2026",title:"The Real Cost of Running AI Agents in Production — A Monthly Breakdown (2026)",description:"Your AI agent costs 5–15× more in production than your prototype. Real token burn rates from Anthropic, 2026 LLM pricing tables, a 3-tier optimization playbook, and self-hosting break-even math.",date:"2026-05-06",tags:["AI Agents","LLM Pricing","Cost Optimization","Production"],readTime:"12 min",editorial:!0,content:`
...
This is the gap nobody warns you about. Prototype costs are measured in dollars. Production costs are measured in thousands. The same workload can cost 47\xd7 more depending on which model you choose. And the difference between a well-optimized deployment and an unoptimized one is the difference between a sustainable product and a burning balance sheet.
...
Pricing varies 600\xd7 across major APIs. From DeepSeek V4 Flash at $0.14 per million input tokens to GPT-5.5 at $5.00. The table below shows the 12 models production agent teams actually choose between—not the full 300+ catalog, but the decision set for real work.
...
The key insight: output tokens cost 4–8\xd7 more than input tokens, and agentic workloads generate *long* outputs. A coding assistant that consumes 100K input tokens might produce 400K output tokens. At Claude Sonnet pricing that's $1,200 in output tokens alone.
...
The multi-agent overhead is worse. Orchestration agents, verification agents, fallback handlers can 10\xd7 token usage versus a well-designed single-agent call. In the ChatDev multi-agent software engineering pipeline, the **Code Review phase consumes 59.4% of all tokens**—not the initial code generation.
...
The model choice is often a **20–140\xd7 cost difference** on the same workload. The question isn't which model is best—it's whether the quality difference matters $24,000/month worth.
...
**Enterprise (100 bots \xd7 50K tokens/day):** ~$2,400/month in token costs on GPT-5.2. Add infrastructure and maintenance: $10,000–$20,000/month.
...
**Trim context windows:** Implement rolling summarization. Agents that accumulate full conversation history are often 10\xd7 the token cost of equivalent task-specific calls.
...
**Fine-tuning for high-volume tasks:** A fine-tuned small model outperforms a prompted frontier model for domain-specific tasks at 10–50\xd7 lower cost per call. One team saved $150/day after 3 weeks of training work—ROI in 6 days.
...
|---|---|---|---|
| 10M tokens | $56 | $3,240 | API (58\xd7 cheaper) |
| 100M tokens | $563 | $3,240 | API (5.8\xd7 cheaper) |
| 256M tokens | $1,440 | $3,240 | API (still cheaper) |
| 500M tokens | $2,813 | $3,240 | Near parity |
| 1B tokens | $5,625 | $3,240 | Self-host (1.7\xd7 cheaper) |
| 3.9B tokens | $21,938 | $3,240 | Self-host (6.8\xd7 cheaper) |
...
> "At 1M tokens/day, self-hosting Llama 3.3 70B on Azure is **733\xd7 more expensive** than the DeepInfra API."

A real healthcare AI team: $10,400/month self-hosted vs $1,870/month if they had used the OpenAI API—paid **5.6\xd7 more** for the privilege of managing infrastructure.
...
**The bandwidth physics:** Token generation speed = memory bandwidth / model size. The M3 Ultra's 819 GB/s vs an RTX 5090's 1,792 GB/s explains the 2–4\xd7 speed gap. For a solo agent serving 1 request at a time: the speed gap barely matters. Response latency of 2–4 seconds is acceptable for many internal workflows.
...
2. **Claude Code burns 4.2\xd7 more tokens per task than Aider or Cursor** on identical work with the same backing model.
...
4. **The 47\xd7 cost spread on the same workload.** Customer support at 1M conversations/month: $180 on Gemini Flash vs $8,400 on Claude Sonnet.
5. **At 1M tokens/day, self-hosting on Azure is 733\xd7 more expensive than DeepInfra API.** Self-hosting answers privacy needs, not cost needs, until you hit massive scale.
...
Production AI agent costs are predictable if you model them correctly. The 5–15\xd7 underestimation from prototype to production happens because teams measure tokens but not the full stack.
...
For technical founders already in production: the question isn't whether you can afford to optimize. It's whether you can afford not to.
    `.trim()},{slug:"ai-newsletter-reads-like-database-dump",title:"Your AI Newsletter Reads Like a Database Dump (Here's the Fix)",description:"Our AI-generated digests had an 18% open rate. Five structural rules from Morning Brew, TLDR, and Ben's Bites doubled it. The template, the prompt trick, and the before/after.",date:"2026-05-05",tags:["AI Agents","Newsletters","Content","Automation"],readTime:"5 min",editorial:!0,content:`
...
After the first week under this structure our open rate rose to thirty-four percent and we received twelve direct replies. The pipeline still runs automatically, but the output no longer reads like a database dump.
    `.trim()},{slug:"pricing-ai-agent-services-solo-studio",title:"How to Price AI Agent Services When Nobody Knows What They Cost",description:"I surveyed 20 agencies and freelancers offering AI agent builds. The range was $500 to $150,000. Here's the framework I use to price Edgeless Lab's services.",date:"2026-05-05",tags:["AI Agents","Pricing","Business","Solo Founder"],readTime:"6 min",editorial:!0,content:`
...
The market will standardize eventually. Right now, the advantage goes to whoever can articulate the value clearly enough to justify the price. That's not a pricing problem. It's a positioning problem.
    `.trim()},{slug:"self-healing-ai-infrastructure",title:"Half My AI Agents Were Dead. I Didn't Know for a Week.",description:"I discovered 10 of my 20 agents were ghosts — registered, visible in dashboards, producing nothing. Here's the five-layer self-healing system I built so it never happens again.",date:"2026-05-05",tags:["Multi-Agent","Infrastructure","Self-Healing","Monitoring"],readTime:"8 min",editorial:!0,productSlug:"multi-agent-blueprint",ctaHook:"The monitoring scripts, escalation protocol, and healing patterns behind this post — ready to drop into your own agent infrastructure.",content:`
...
*Edgeless Lab builds infrastructure for autonomous AI systems.*
    `.trim()},{slug:"what-portland-businesses-need-from-ai",title:"I Read 500 Google Reviews in Portland. Here's What Small Businesses Actually Need from AI.",description:"Dental offices losing patients to voicemail. HVAC companies quoting three days late. The AI solutions Portland small businesses need aren't chatbots. They're answering the phone.",date:"2026-05-05",tags:["AI Agents","Local Business","Voice AI","Portland"],readTime:"6 min",editorial:!0,content:`
...
The model will hallucinate. The file system will have race conditions. The network will fail. The question isn't whether your agent will get grounded incorrectly. It's whether your system detects it before it matters.
    `.trim()},{slug:"ai-agent-never-sleeps-hermes-vps",title:"The AI Agent That Never Sleeps: Running Hermes 24/7 on a $5 VPS",description:"Most AI agents die when you close the laptop. Hermes runs 24/7 on a Hetzner VPS in Helsinki, handling email, triaging knowledge, and monitoring systems while I sleep.",date:"2026-04-29",tags:["Hermes","AI Agents","VPS","Infrastructure"],readTime:"7 min",editorial:!0,productSlug:"multi-agent-blueprint",ctaHook:"The dispatch pattern, bus protocol, and reference implementations for building your own always-on agent system.",content:`
...
That's the difference between using AI and having AI infrastructure.
    `.trim()},{slug:"claude-code-hooks-harness-engineering",title:"Claude Code Hooks: The Harness Engineering That Actually Matters",description:"Everyone's optimizing their prompts. The real leverage is in the 200 lines of Python that run before and after every tool call.",date:"2026-04-29",tags:["Claude Code","Hooks","Agent Safety","Python"],readTime:"6 min",editorial:!0,productSlug:"hooks-deep-dive",ctaHook:"11 production hook implementations, shared libraries, and configuration templates you can drop into any Claude Code project.",content:`
...
Each hook makes the system slightly more trustworthy. More trust means more autonomy. More autonomy means more real-world exposure, which reveals more failure modes, which means more hooks. The model keeps getting better on its own. The harness is the part only you can build.
    `.trim()},{slug:"12-dollar-ai-operations-team",title:"I Run a $12/Week AI Operations Team. Here's the Cost Breakdown.",description:"Enterprise AI ops costs $50K+/month. I run 5 agents, 24/7, for $12/week. The architecture, the model routing, and why cheap doesn't mean fragile.",date:"2026-04-19",tags:["Multi-Agent","AI Infrastructure","Cost Optimization","Claude Code"],readTime:"8 min",editorial:!0,isLaunch:!0,productSlug:"multi-agent-blueprint",ctaHook:"The complete Paperclip OS blueprint: configs, routing logic, and the cost calculator behind this post.",content:`
...
The $12/week isn't the point. The point is that cheap infrastructure can be reliable if you design for the actual failure modes instead of the theoretical ones.
    `.trim()},{slug:"when-plaid-becomes-tartan",title:"When Does Generated Plaid Become Tartan?",description:"Six weave structures, 48 period-correct dye colors, and one question the Scottish Register cannot answer. A field journal on building a generative tartan engine.",date:"2026-04-16",tags:["Generative Art","Creative Coding","Textiles","SVG"],readTime:"7 min",editorial:!0,productSlug:"gen-art-starter",ctaHook:"10 generators, parameter guides, and the scoring rubric from 105+ experiments.",content:`
...
At what point does generated plaid become tartan? I do not have an answer. But the question is worth a field journal, and the [full editorial with interactive generator](/tartanism/) is the journal.
    `.trim()},{slug:"ninety-six-algorithms-one-constraint",title:"96 Algorithms, One Constraint: A Pen on Paper",description:"A taxonomy of every generative art algorithm that survives the pen plotter constraint. Flow fields to fractals, reaction-diffusion to recursive trees. The catalog, the surprises, and what categories produce the best physical output.",date:"2026-04-15",tags:["Generative Art","Creative Coding","Pen Plotters","Algorithms"],readTime:"8 min",editorial:!0,productSlug:"gen-art-starter",ctaHook:"The 10 best generators from this catalog, ready to run with parameter guides.",content:`
...
Whatever shows up next in that repo, I'll approach it the same way: read it, steal the good parts, push back on the rest.
    `.trim()},{slug:"shipped-7-products-in-7-days",title:"I Shipped 7 Digital Products in 7 Days. Here's Exactly How.",description:"The meta-narrative: how one solo developer used AI agents, autoreason scoring, and a daily shipping cadence to go from 11 to 18 products in a week.",date:"2026-04-09",tags:["Solo Dev","Products","Process"],readTime:"6 min",editorial:!0,isLaunch:!0,productSlug:"launch-toolkit",ctaHook:"The exact templates, pricing model, and launch checklist from this 7-day sprint.",content:`
...
Everything on the [products page](/products).
    `.trim()},{slug:"generative-art-algorithms-that-work",title:"I Built 75 Generative Art Algorithms. These 10 Actually Look Good.",description:"Most generative art looks like noise. After 105+ experiments with pen plotters and AI scoring, these are the algorithms that consistently produce work worth framing.",date:"2026-04-08",tags:["Generative Art","Pen Plotters","Creative Coding"],readTime:"6 min",editorial:!0,isLaunch:!0,productSlug:"gen-art-starter",ctaHook:"Working generators for every algorithm in this post, plus SVG optimization scripts.",content:`
...
The best generative art doesn't look generative. It looks like someone made a deliberate choice at every point. The algorithm just happens to be the one making those choices.
    `.trim()},{slug:"agents-that-talk-to-each-other",title:"How I Run 5 AI Agents That Talk to Each Other",description:"A dispatch agent routes tasks to specialist workers. They communicate through a real-time bus and async inboxes. The architecture, and why most multi-agent frameworks get it wrong.",date:"2026-04-07",tags:["Multi-Agent","Architecture","Claude Code"],readTime:"7 min",editorial:!0,isLaunch:!0,productSlug:"multi-agent-blueprint",ctaHook:"The dispatch pattern, bus protocol, and 3 reference implementations from this architecture.",content:`
...
The value isn't in the architecture diagram. It's in knowing which shortcuts work and which ones break at 3am.
    `.trim()},{slug:"n8n-workflows-ai-business",title:"5 n8n Workflows That Run My AI Business",description:"Visual automation for solo developers. How I use n8n to monitor YouTube, digest RSS feeds, review code, and pipe everything through Claude without writing a scheduler.",date:"2026-04-06",tags:["n8n","Automation","Workflows"],readTime:"5 min",editorial:!0,isLaunch:!0,productSlug:"n8n-ai-workflows",ctaHook:"Importable JSON workflows, env configs, and setup guides for all 5 automations.",content:`
...
The full workflow JSON files, setup guides, and customization instructions are in the [n8n AI Workflow Templates](/products) on the products page.
    `.trim()},{slug:"mcp-servers-break-in-production",title:"Most MCP Servers Break in Production. The 5 Failure Modes I Found.",description:"400+ MCP servers exist. Most work in demos and fail under real load. The 5 failure modes I hit running MCP servers 24/7, and what production-grade actually means.",date:"2026-04-05",tags:["MCP","Infrastructure","Production"],readTime:"5 min",editorial:!0,isLaunch:!0,productSlug:"production-mcp-kit",ctaHook:"Auth middleware, rate limiting, health checks, and Docker configs that survived production.",content:`
...
The gap between "works in a demo" and "runs unattended at 3am" is where most MCP servers live. Closing that gap isn't glamorous work. But it's the work that matters.
    `.trim()},{slug:"agent-lost-252-dollars",title:"I Let an AI Agent Move My Money. It Lost $252.",description:"An autonomous agent exceeded its scope, moved funds without verification, and then lied about recovery. The full post-mortem, and the 3 guardrails that would have prevented it.",date:"2026-04-04",tags:["AI Safety","Agents","Post-Mortem"],readTime:"6 min",editorial:!0,isLaunch:!0,productSlug:"agent-safety-patterns",ctaHook:"10 anti-patterns, scope containment hooks, and the financial verification protocol from this incident.",content:`
...
Your job isn't to trust the agent. It's to make the wrong path impossible.
    `.trim()},{slug:"the-hook-that-saved-my-codebase",title:"The Hook That Saved My Codebase",description:"A single Claude Code hook prevented a cascading rm -rf from wiping source files. The damage-control pattern, and 3 hooks you can steal today.",date:"2026-04-03",tags:["Claude Code","Hooks","Developer Tools"],readTime:"5 min",editorial:!0,isLaunch:!0,productSlug:"hooks-deep-dive",ctaHook:"10 production hooks, composition patterns, and the damage-control system from this post.",content:`
...
The hooks that matter most aren't the clever ones. They're the boring ones that run thousands of times and catch the one mistake that would have cost you a day of work.
    `.trim()},{slug:"pretext-typography-that-thinks",title:"PreText: Typography That Thinks",description:"Most web text is a dumb rectangle. PreText measures text before rendering, enabling layouts CSS literally cannot express. Six ways we use it.",date:"2026-04-02",tags:["PreText","Typography","Web Development"],readTime:"6 min",editorial:!0,content:`
...
The [Prompt Engineering OS](/products) covers 30 chapters of patterns like these, with 100+ templates you can adapt. The [Quick Reference Cards](/products) distill the most critical patterns into printable cheat sheets.
    `.trim()},{slug:"one-file-memory-system",title:"The One-File Memory System That Changed How I Use Claude",description:"You shouldn't have to re-explain your stack every session. The simplest possible setup to give Claude persistent memory, and how to do it in 10 minutes.",date:"2026-03-26",tags:["Claude Code","Productivity","Memory"],readTime:"4 min",editorial:!0,content:`
...
See the [lab experiments page](/lab) for the MCP servers running in this system, or read the [infrastructure deep-dive](/blog/building-ai-agent-infrastructure-solo) for the full architecture.
    `.trim()},{slug:"generative-art-pen-plotters",title:"Generative Art for Pen Plotters: A Technical Primer",description:"Pen plotter art isn't screen art printed on paper. The constraints change everything: single-stroke paths, pen-up/pen-down optimization, and SVG as the lingua franca.",date:"2026-03-23",tags:["Generative Art","Creative Coding","Pen Plotters"],readTime:"7 min",editorial:!0,productSlug:"gen-art-starter",ctaHook:"10 plotter-ready generators with SVG optimization and AI scoring rubrics.",content:`
...
If you want to go deeper into the scoring and iteration pipeline, the [pen plotter autoresearch pattern](/lab) documents how the AI-in-the-loop workflow runs.
    `.trim()},{slug:"building-ai-agent-infrastructure-solo",title:"Building AI Agent Infrastructure as a Solo Developer",description:"How I built a multi-agent system with MCP servers, vector memory, and autonomous trading, all running 24/7 from a single VPS.",date:"2026-03-21",tags:["Agents","MCP","Infrastructure"],readTime:"8 min",editorial:!0,content:`
...
The goal isn't to build the most complex system. It's to build the most useful one, with the least moving parts.
    `.trim()},{slug:"how-claude-code-memory-works",title:"How Claude Code Memory Actually Works",description:"Claude forgets everything between sessions. File-based memory fixes that. The simplest setup, and why it changes how you work with AI.",date:"2026-03-21",tags:["Claude Code","Memory","Developer Tools"],readTime:"6 min",editorial:!0,content:`
...
The best time to set up memory is before your next session. Takes 15 minutes, saves hours every week.
    `.trim()}].filter(e=>"draft"!==e.status),A="One developer shipping autonomous agents, MCP servers, and generative art. 18 products, all free. Everything open source.",I='300 14px "Geist"';e.s(["AboutBlurb",0,function(){let e=(0,a.useRef)(null),[n,i]=(0,a.useState)(0);(0,a.useEffect)(()=>{if(!e.current)return;let t=()=>{e.current&&i(e.current.clientWidth)};t();let a=new ResizeObserver(t);return a.observe(e.current),()=>a.disconnect()},[]);let r=function(e,t,s){let{ready:o,prepare:n,layout:i,prepareWithSegments:r,walkLineRanges:l}=(0,g.usePreText)("Geist"),[h,c]=(0,a.useState)(null),d=(0,a.useRef)(""),u=(0,a.useCallback)(()=>{if(!o||!n||!i||!r||!l||s<=0)return;let a=`${e}::${t}::42::${s}`;if(a===d.current)return;d.current=a;let h=n(e,t);if(!h)return;let u=i(h,s,42);if(!u||u.lineCount<=1){let a=r(e,t);if(!a)return;let o=0;l(a,s,e=>{e.width>o&&(o=e.width)}),c(Math.ceil(o));return}let p=r(e,t);if(!p)return;let m=0;l(p,s,e=>{e.width>m&&(m=e.width)});let g=Math.ceil(m),f=i(h,g,42);if(f&&f.lineCount>u.lineCount)for(;g<s;){let e=i(h,++g,42);if(e&&e.lineCount<=u.lineCount)break}c(g)},[o,e,t,42,s,n,i,r,l]);return(0,a.useEffect)(()=>{u()},[u]),h}("One person shipping agents, MCP servers, generative art, and developer tools. In production, in the open, since day one. No pitch decks. No vaporware.",'300 28px "Geist"',n);return(0,t.jsxs)("div",{className:"max-w-2xl",ref:e,children:[(0,t.jsxs)("p",{className:"text-2xl sm:text-3xl font-light leading-[1.5]",style:{color:"var(--text-secondary)",animation:"fadeInUp 0.6s cubic-bezier(0.16,1,0.3,1) both",...r?{maxWidth:`${r}px`}:{}},children:[(0,t.jsx)("span",{style:{color:"var(--text-primary)"},children:"One person"})," shipping agents, MCP servers, generative art, and developer tools. In production, in the open, since day one."," ",(0,t.jsx)("span",{style:{color:"var(--text-primary)"},children:"No pitch decks. No vaporware."})]}),(0,t.jsx)("div",{className:"mt-8",style:{animation:"fadeInUp 0.4s cubic-bezier(0.16,1,0.3,1) 0.2s both"},children:(0,t.jsxs)(s.default,{href:"/about",className:"text-sm font-medium flex items-center gap-1.5 transition-colors hover:text-[var(--text-primary)]",style:{color:"var(--accent)"},children:["About the lab ",(0,t.jsx)(o,{size:14})]})})]})},"CapabilitiesGrid",0,function({capabilities:e}){return(0,t.jsx)("div",{className:"grid grid-cols-1 md:grid-cols-2 gap-4",children:e.map((e,a)=>(0,t.jsxs)("div",{className:"rounded-xl border overflow-hidden",style:{background:"var(--bg-base)",borderColor:"var(--border-subtle)",animation:`fadeInUp 0.4s cubic-bezier(0.16,1,0.3,1) ${.08*a}s both`},children:[(0,t.jsxs)("div",{className:"px-5 py-3 border-b flex items-center justify-between",style:{borderColor:"var(--border-subtle)"},children:[(0,t.jsx)("span",{className:"text-xs font-mono",style:{color:"var(--text-secondary)"},children:e.label}),(0,t.jsx)("span",{className:"w-1.5 h-1.5 rounded-full",style:{background:"var(--green)"}})]}),(0,t.jsx)("pre",{className:"px-5 py-4 text-xs leading-[1.8] font-mono whitespace-pre overflow-x-auto",style:{color:"var(--green)"},children:e.snippet})]},e.label))})},"ExperimentsGrid",0,function({experiments:e}){return(0,t.jsx)("div",{className:"grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4",children:e.map((e,a)=>(0,t.jsxs)("a",{href:e.href,...e.external?{target:"_blank",rel:"noopener noreferrer"}:{},className:"group relative rounded-xl border p-5 transition-all hover:scale-[1.02] flex flex-col",style:{background:"var(--bg-surface)",borderColor:"var(--border-subtle)",animation:`fadeInUp 0.4s cubic-bezier(0.16,1,0.3,1) ${.08*a}s both`},children:[(0,t.jsxs)("div",{className:"flex items-center justify-between mb-3",children:[(0,t.jsx)("span",{className:"text-xs font-mono uppercase tracking-[0.12em]",style:{color:"var(--accent)"},children:e.category}),e.status&&(0,t.jsx)("span",{className:"text-label font-mono uppercase tracking-[0.1em] px-1.5 py-0.5 rounded",style:{background:"Live"===e.status?"var(--green-muted)":"var(--accent-muted)",color:"Live"===e.status?"var(--green)":"var(--accent)"},children:e.status})]}),(0,t.jsx)("span",{className:"text-sm font-medium block mb-2",style:{color:"var(--text-primary)"},children:e.title}),e.description&&(0,t.jsx)("p",{className:"text-xs line-clamp-2 mb-3 flex-1",style:{color:"var(--text-tertiary)",lineHeight:1.6},children:e.description}),e.stack&&e.stack.length>0&&(0,t.jsx)("div",{className:"flex flex-wrap gap-1.5 mt-auto",children:e.stack.map(e=>(0,t.jsx)("span",{className:"text-label font-mono px-1.5 py-0.5 rounded",style:{background:"rgba(255,255,255,0.05)",color:"var(--text-tertiary)"},children:e},e))}),(0,t.jsx)("div",{className:"absolute top-5 right-5 opacity-0 group-hover:opacity-100 transition-opacity",style:{color:"var(--text-tertiary)"},children:(0,t.jsx)(n.ArrowUpRight,{size:14})})]},e.title))})},"FeaturedGrid",0,function({projects:e}){return(0,t.jsx)("div",{className:"grid grid-cols-1 md:grid-cols-3 gap-4 md:grid-rows-[auto_auto]",children:e.map((e,a)=>(0,t.jsx)("div",{className:e.span,children:(0,t.jsx)(c,{className:"h-full",href:e.href,children:(0,t.jsxs)("div",{style:{animation:`fadeInUp 0.5s cubic-bezier(0.16,1,0.3,1) ${.1*a}s both`},children:[(0,t.jsxs)("div",{className:"w-full rounded-lg mb-6 overflow-hidden",style:{background:"rgba(0,0,0,0.4)",border:"1px solid var(--border-subtle)"},children:[(0,t.jsxs)("div",{className:"flex items-center gap-1.5 px-3 py-2.5 border-b",style:{borderColor:"var(--border-subtle)"},children:[(0,t.jsx)("div",{className:"w-2 h-2 rounded-full",style:{background:"rgba(255,255,255,0.1)"}}),(0,t.jsx)("div",{className:"w-2 h-2 rounded-full",style:{background:"rgba(255,255,255,0.1)"}}),(0,t.jsx)("div",{className:"w-2 h-2 rounded-full",style:{background:"rgba(255,255,255,0.1)"}}),(0,t.jsx)("span",{className:"ml-2 text-xs font-mono",style:{color:"var(--text-tertiary)"},children:e.title.toLowerCase().replace(/\s+/g,"-")})]}),(0,t.jsx)("pre",{className:`px-3 py-3 text-xs leading-[1.7] font-mono whitespace-pre overflow-hidden ${0===a?"min-h-[120px]":"min-h-[80px]"}`,style:{color:"var(--green)"},children:e.snippet})]}),(0,t.jsx)("div",{className:"flex items-start justify-between gap-4",children:(0,t.jsxs)("div",{children:[(0,t.jsx)("h3",{className:"text-lg font-semibold mb-1.5",style:{color:"var(--text-primary)"},children:e.title}),(0,t.jsx)("p",{className:"text-sm max-w-md",style:{color:"var(--text-secondary)",lineHeight:1.6},children:e.description})]})}),(0,t.jsx)("div",{className:"flex flex-wrap gap-2 mt-4",children:e.tags.map(e=>(0,t.jsx)("span",{className:"px-2.5 py-1 text-xs font-mono rounded-md",style:{background:"var(--accent-muted)",color:"var(--accent)"},children:e},e))})]})})},e.title))})},"HeroSection",0,function(){return(0,t.jsxs)("section",{className:"relative flex min-h-[92svh] items-center px-6 pb-16 pt-28 md:min-h-screen md:items-end md:pb-24 md:pt-32 texture-grain texture-scanlines overflow-hidden",children:[(0,t.jsx)(p,{}),(0,t.jsxs)("div",{className:"relative max-w-[1280px] w-full mx-auto grid grid-cols-1 gap-12 lg:grid-cols-[1.25fr_1fr] lg:items-end",children:[(0,t.jsxs)("div",{className:"min-w-0",children:[(0,t.jsx)(h,{children:(0,t.jsx)("div",{className:"inline-flex items-center gap-2 mb-8 px-3 py-1.5 border",style:{borderColor:"rgba(57, 255, 20, 0.2)",background:"rgba(57, 255, 20, 0.04)",borderRadius:0},children:(0,t.jsx)("span",{className:"text-small font-mono uppercase tracking-[0.14em]",style:{color:"var(--phosphor)"},children:"[SYS] 18 products online · shipping daily"})})}),(0,t.jsxs)("h1",{className:"text-[clamp(3rem,8vw,7.5rem)] font-bold leading-[0.88] tracking-[-0.04em]",style:{color:"var(--text-primary)"},children:[(0,t.jsx)(l,{text:"Built solo",delay:.1}),(0,t.jsx)("br",{}),(0,t.jsx)("span",{style:{color:"var(--accent)"},children:(0,t.jsx)(l,{text:"Shipped",delay:.3})})," ",(0,t.jsx)(l,{text:"open",delay:.45})]}),(0,t.jsx)(h,{delay:.7,children:(0,t.jsx)("div",{className:"mt-8 max-w-xl",children:(0,t.jsx)(f,{text:A,font:'300 18px "Geist"',lineHeight:28,cursorRadius:36,cursorColor:"var(--accent)",className:"text-lg font-light",style:{color:"var(--text-secondary)"},fallback:(0,t.jsx)("p",{style:{lineHeight:1.55},children:A})})})}),(0,t.jsx)(h,{delay:.85,children:(0,t.jsxs)("div",{className:"mt-8 flex items-center gap-2.5 max-w-xl text-caption font-mono",style:{color:"var(--text-tertiary)"},children:[(0,t.jsx)("span",{className:"px-2 py-0.5 rounded uppercase tracking-[0.12em]",style:{background:"var(--accent-muted)",color:"var(--accent)"},children:"Now"}),(0,t.jsxs)("span",{children:["Shipping"," ",(0,t.jsx)(s.default,{href:"/products/launch-toolkit",className:"underline-offset-2 hover:underline transition-colors hover:text-[var(--text-primary)]",style:{color:"var(--text-secondary)"},children:"Digital Product Launch Toolkit"})," ","· 7 products in 7 days"]})]})}),(0,t.jsx)(h,{delay:.9,children:(0,t.jsxs)("div",{className:"mt-8 flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:gap-5",children:[(0,t.jsxs)(s.default,{href:"/products",className:"inline-flex items-center gap-2 h-11 px-6 text-sm font-medium text-white rounded-full transition-all hover:brightness-110 hover:scale-[1.02]",style:{background:"var(--accent-solid)"},onClick:()=>r("hero_view_products","/products"),children:["18 free products ",(0,t.jsx)(o,{size:15})]}),(0,t.jsxs)(s.default,{href:"/projects",className:"inline-flex items-center gap-2 h-11 px-6 text-sm font-medium rounded-full border transition-all hover:brightness-110 hover:scale-[1.02]",style:{color:"var(--text-secondary)",borderColor:"var(--border-subtle)"},onClick:()=>r("hero_view_projects","/projects"),children:["See what’s running ",(0,t.jsx)(o,{size:15})]}),(0,t.jsxs)("a",{href:"https://github.com/edgeless-ai",className:"text-sm font-medium flex items-center gap-1.5 transition-colors hover:text-[var(--text-primary)]",style:{color:"var(--text-secondary)"},children:["GitHub ",(0,t.jsx)(n.ArrowUpRight,{size:14})]})]})})]}),(0,t.jsx)(h,{delay:.5,children:(0,t.jsx)("div",{className:"hidden lg:block",children:(0,t.jsx)(m.GenerativeAscii,{})})})]})]})},"ProductHighlight",0,function(){let e=T.filter(e=>"Free"!==e.price).slice(0,6),a=T.find(e=>"Free"===e.price),i=T.length-e.length-!!a;return(0,t.jsxs)("div",{className:"grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3",children:[e.map((e,a)=>(0,t.jsxs)("a",{href:e.href,target:"_blank",rel:"noopener noreferrer",className:"group block rounded-xl border p-5 transition-all hover:scale-[1.01] hover:border-[var(--border-hover)]",style:{background:"var(--bg-surface)",borderColor:"var(--border-subtle)",animation:`fadeInUp 0.4s cubic-bezier(0.16,1,0.3,1) ${.08*a}s both`},children:[(0,t.jsxs)("div",{className:"flex items-center justify-between mb-2",children:[(0,t.jsx)("span",{className:"text-sm font-semibold",style:{color:"var(--text-primary)"},children:e.name}),(0,t.jsx)(n.ArrowUpRight,{size:14,className:"opacity-0 group-hover:opacity-100 transition-opacity",style:{color:"var(--text-tertiary)"}})]}),(0,t.jsx)("span",{className:"text-lg font-bold font-mono block mb-2",style:{color:"var(--accent)"},children:e.price}),(0,t.jsx)("p",{className:"text-xs",style:{color:"var(--text-tertiary)",lineHeight:1.6},children:e.description})]},e.name)),a&&(0,t.jsxs)(s.default,{href:"/products/",className:"group block rounded-xl border p-5 transition-all hover:scale-[1.01] hover:border-[var(--border-hover)]",style:{background:"var(--bg-surface)",borderColor:"var(--border-subtle)",animation:"fadeInUp 0.4s cubic-bezier(0.16,1,0.3,1) 0.24s both"},children:[(0,t.jsxs)("div",{className:"flex items-center justify-between mb-2",children:[(0,t.jsx)("span",{className:"text-sm font-semibold",style:{color:"var(--text-primary)"},children:a.name}),(0,t.jsx)("span",{className:"text-xs font-mono px-2 py-0.5 rounded-md",style:{background:"rgba(34,197,94,0.15)",color:"var(--green)"},children:"Free"})]}),(0,t.jsx)("p",{className:"text-xs mb-3",style:{color:"var(--text-tertiary)",lineHeight:1.6},children:a.description}),(0,t.jsxs)("span",{className:"text-xs font-medium flex items-center gap-1",style:{color:"var(--accent)"},children:["+",i," more products ",(0,t.jsx)(o,{size:12})]})]})]})},"RecentActivity",0,function(){let[e,o]=(0,a.useState)([]);return(0,a.useEffect)(()=>{let e=[...x].sort((e,t)=>new Date(t.date).getTime()-new Date(e.date).getTime()).map(e=>({id:e.slug,type:e.isLaunch?"launch":"post",title:e.title,date:e.date,href:`/blog/${e.slug}`}));fetch("/site-activity.json").then(e=>e.ok?e.json():null).then(t=>{t?.items?o([...t.items.map(e=>({id:e.id,type:"ship"===e.type?"ship":"wip",title:e.title,date:e.date,href:"/blog",agent:e.agent})),...e].sort((e,t)=>new Date(t.date).getTime()-new Date(e.date).getTime()).filter((e,t,a)=>a.findIndex(t=>t.id===e.id)===t).slice(0,10)):o(e.slice(0,8))}).catch(()=>{o(e.slice(0,8))})},[]),(0,t.jsx)("ul",{className:"divide-y",style:{borderColor:"var(--border-subtle)"},children:e.map((e,a)=>{let o,n,i=function(e){switch(e){case"launch":case"wip":return{background:"var(--accent-muted)",color:"var(--accent)"};case"ship":return{background:"var(--green-muted)",color:"var(--green)"};default:return{background:"var(--bg-surface)",color:"var(--text-tertiary)"}}}(e.type);return(0,t.jsx)("li",{style:{borderColor:"var(--border-subtle)",animation:`fadeInUp 0.4s cubic-bezier(0.16,1,0.3,1) ${.05*a}s both`},className:"border-b first:border-t",children:(0,t.jsxs)(s.default,{href:e.href,className:"group grid grid-cols-[auto_auto_1fr_auto] items-center gap-4 py-4 px-1 transition-colors",children:[(0,t.jsx)("span",{className:"text-small font-mono tabular-nums shrink-0 w-[68px]",style:{color:"var(--text-tertiary)"},children:e.date.slice(5,10)}),(0,t.jsx)("span",{className:"text-label font-mono uppercase tracking-[0.12em] px-2 py-0.5 rounded shrink-0",style:{...i,border:"1px solid var(--border-subtle)"},children:function(e){switch(e){case"launch":return"launch";case"ship":return"ship";case"wip":return"wip";default:return"post"}}(e.type)}),(0,t.jsx)("span",{className:"text-body-sm font-medium truncate transition-colors group-hover:text-[var(--text-primary)]",style:{color:"var(--text-primary)"},children:e.agent&&"post"!==e.type&&"launch"!==e.type?`${e.agent}: ${e.title}`:e.title}),(0,t.jsx)("span",{className:"text-small font-mono shrink-0 hidden sm:inline",style:{color:"var(--text-tertiary)"},children:(o=new Date(e.date).getTime(),(n=Math.floor((Date.now()-o)/864e5))<1?"today":1===n?"yesterday":n<30?`${n}d ago`:n<365?`${Math.floor(n/30)}mo ago`:`${Math.floor(n/365)}y ago`)})]})},e.id)})})},"StackFlow",0,function({nodes:e}){let s=(0,a.useMemo)(()=>(function(e){let t=[];for(let a=0;a<e.length;a++){let s=e[a];t.push({text:s.label,font:'600 14px "Geist Mono"',lineHeight:28,style:{color:"var(--accent)"}}),t.push({text:`\u00A0${s.sublabel}`,font:I,lineHeight:28,style:{color:"var(--text-tertiary)"}}),a<e.length-1&&t.push({text:" → ",font:I,lineHeight:28,style:{color:"var(--border-focus)"}})}return t})(e),[e]);return(0,t.jsxs)(t.Fragment,{children:[(0,t.jsx)("div",{className:"max-w-2xl",children:(0,t.jsx)(w,{segments:s,style:{color:"var(--text-tertiary)"}})}),(0,t.jsx)("div",{className:"mt-8 max-w-2xl",children:(0,t.jsx)(b,{text:"Agents call tools through MCP. Knowledge persists across sessions in vector memory and markdown vaults. A single VPS keeps the whole system running, unattended. Every tool is an MCP server. Every agent can use any tool. Add a new capability and it's immediately available to every agent in the system.",font:'300 14px "Geist"',lineHeight:24,orbCount:3,orbRadius:36,textColor:"var(--text-tertiary)"})})]})},"SubscribeSection",0,function(){return(0,t.jsx)("section",{className:"px-6 py-20 border-t",style:{borderColor:"var(--border-subtle)"},children:(0,t.jsx)("div",{className:"max-w-[1280px] mx-auto",children:(0,t.jsx)("div",{className:"max-w-lg",children:(0,t.jsxs)("div",{style:{animation:"fadeInUp 0.4s cubic-bezier(0.16,1,0.3,1) both"},children:[(0,t.jsxs)("div",{className:"flex items-center gap-2 mb-4",children:[(0,t.jsx)("span",{className:"w-1.5 h-1.5 rounded-full",style:{background:"var(--green)"}}),(0,t.jsx)("span",{className:"text-xs font-mono",style:{color:"var(--text-tertiary)"},children:"Stay in the loop"})]}),(0,t.jsx)("h2",{className:"text-2xl font-semibold tracking-tight mb-2",style:{color:"var(--text-primary)"},children:"Everything ships on GitHub."}),(0,t.jsx)("p",{className:"text-sm mb-6",style:{color:"var(--text-secondary)",lineHeight:1.7},children:"Agent frameworks, generative art, and developer tools - all in the open."}),(0,t.jsxs)("a",{href:"https://github.com/edgeless-ai",target:"_blank",rel:"noopener noreferrer",className:"inline-flex items-center gap-2 h-11 px-6 text-sm font-medium text-white rounded-full transition-all hover:brightness-110 hover:scale-[1.02]",style:{background:"var(--accent-solid)"},children:["GitHub ",(0,t.jsx)(n.ArrowUpRight,{size:14})]})]})})})})}],43875)},62481,e=>{"use strict";var t=e.i(26869),a=e.i(14363);let s=(0,e.i(72592).default)("package",[["path",{d:"M11 21.73a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73z",key:"1a0edw"}],["path",{d:"M12 22V12",key:"d0xqtd"}],["polyline",{points:"3.29 7 12 12 20.71 7",key:"ousv84"}],["path",{d:"m7.5 4.27 9 5.15",key:"1c824w"}]]);function o(){return(0,t.jsx)("ul",{className:"divide-y",style:{borderColor:"var(--border-subtle)"},children:[0,1,2].map(e=>(0,t.jsx)("li",{className:"py-4 px-1 border-b",style:{borderColor:"var(--border-subtle)"},children:(0,t.jsxs)("div",{className:"flex items-center gap-4",children:[(0,t.jsx)("div",{className:"h-4 w-16 rounded animate-pulse",style:{background:"var(--bg-surface)"}}),(0,t.jsx)("div",{className:"h-4 w-12 rounded animate-pulse",style:{background:"var(--bg-surface)"}}),(0,t.jsx)("div",{className:"h-4 flex-1 rounded animate-pulse",style:{background:"var(--bg-surface)"}})]})},e))})}e.s(["RecentlyShipped",0,function(){let[e,n]=(0,a.useState)(null),[i,r]=(0,a.useState)("live"),[l,h]=(0,a.useState)(!0);return((0,a.useEffect)(()=>{let e=!1;async function t(){try{let t=await fetch("/api/shipped",{headers:{accept:"application/json"},cache:"no-store"});if(!t.ok)throw Error(`HTTP ${t.status}`);let a=await t.json();e||(n(a.items??[]),r(a.source??"live"))}catch{e||(n([{identifier:"EDGA-3653",title:"Recently Shipped Widget — live on edgelesslab.com",completedAt:new Date().toISOString()},{identifier:"EDGA-3517",title:"Edgeless Website Goal Loop",completedAt:"2026-05-17T15:01:17.987Z"},{identifier:"EDGA-3629",title:"[COO Sweep] Unassigned critical TODO issue",completedAt:"2026-05-17T15:00:18.903Z"}]),r("fallback"))}finally{e||h(!1)}}t();let a=()=>{document.hidden||t()};document.addEventListener("visibilitychange",a);let s=setInterval(t,36e5);return()=>{e=!0,document.removeEventListener("visibilitychange",a),clearInterval(s)}},[]),l)?(0,t.jsx)(o,{}):(0,t.jsxs)("div",{children:[(0,t.jsx)("ul",{className:"divide-y",style:{borderColor:"var(--border-subtle)"},children:(e??[]).map((e,a)=>(0,t.jsx)("li",{className:"border-b first:border-t",style:{borderColor:"var(--border-subtle)",animation:`fadeInUp 0.4s cubic-bezier(0.16,1,0.3,1) ${.05*a}s both`},children:(0,t.jsxs)("div",{className:"group grid grid-cols-[auto_auto_1fr_auto] items-center gap-4 py-4 px-1 transition-colors",children:[(0,t.jsx)("span",{className:"text-small font-mono tabular-nums shrink-0 w-[68px]",style:{color:"var(--text-tertiary)"},children:e.completedAt?e.completedAt.slice(5,10):"—"}),(0,t.jsxs)("span",{className:"text-label font-mono uppercase tracking-[0.12em] px-2 py-0.5 rounded shrink-0 flex items-center gap-1",style:{background:"var(--accent-muted)",color:"var(--accent)",border:"1px solid var(--border-subtle)"},children:[(0,t.jsx)(s,{size:10}),"shipped"]}),(0,t.jsx)("span",{className:"text-body-sm font-medium truncate transition-colors group-hover:text-[var(--text-primary)]",style:{color:"var(--text-primary)"},children:e.title}),(0,t.jsx)("span",{className:"text-small font-mono shrink-0 hidden sm:inline",style:{color:"var(--text-tertiary)"},children:function(e){if(!e)return"recently";let t=new Date(e).getTime(),a=Math.floor((Date.now()-t)/864e5);return a<1?"today":1===a?"yesterday":a<30?`${a}d ago`:a<365?`${Math.floor(a/30)}mo ago`:`${Math.floor(a/365)}y ago`}(e.completedAt)})]})},e.identifier))}),"fallback"===i&&(0,t.jsx)("p",{className:"mt-3 text-small font-mono",style:{color:"var(--text-tertiary)"},children:"Offline mode — showing cached shipments."})]})}],62481)},60377,e=>{"use strict";var t=e.i(26869),a=e.i(14363);e.s(["ScrollReveal",0,function({children:e,className:s="",delay:o=0}){let[n,i]=function(e=.15){let t=(0,a.useRef)(null),[s,o]=(0,a.useState)(!1);return(0,a.useEffect)(()=>{let a=t.current;if(!a)return;if(window.matchMedia("(prefers-reduced-motion: reduce)").matches)return void o(!0);let s=new IntersectionObserver(([e])=>{e.isIntersecting&&(o(!0),s.disconnect())},{threshold:e});return s.observe(a),()=>s.disconnect()},[e]),[t,s]}(.12);return(0,t.jsx)("div",{ref:n,className:s,style:{opacity:+!!i,transform:i?"translateY(0)":"translateY(24px)",transition:`opacity 0.6s cubic-bezier(0.16, 1, 0.3, 1) ${o}s, transform 0.6s cubic-bezier(0.16, 1, 0.3, 1) ${o}s`},children:e})}],60377)}]);