...
### ✅ FIXED (deployed+verified 2026-07-03) — retail floor now enforced
*Was: URGENT live margin bug. Now closed: baked designs price at the authoritative _KIND_PRICE_FLOOR[kind]; tee/hoodie variant cross-check (cached classifier) closes the mislabel bypass; pre-charge hoodie-variant reject. Live-probed: $26 tee→$34, $26 tee+hoodie-variant→$48, hoodie-no-variant→400, legit tee unchanged. Adversarially reviewed twice.*

<details><summary>original finding</summary>
A **$26 request on a $34 listed tee produced a real Stripe session for $26** (verified live via `session.amount_total`). CONFIRMED root cause (not deploy-lag — `ca4019f` deployed 2026-07-01): `_retail_price_cents_for_slug()` → `_find_submission(slug)` only iterates the live `SUBMISSIONS` store and returns None for **baked `designs.json` listings — which are the *bulk* of the catalog** (all Foundry designs, the originals, the dog-food). So there is **no server-side retail floor on most of the store**; any baked listing can be under-quoted above POD cost but below shelf price via a tampered `amount_cents`. The cost floor still prevents selling below cost (no guaranteed loss), so the exposure is **margin erosion, not loss** — but it's live. **FIX NEEDS A DESIGN CALL (money-path, gated):** the backend has no authoritative price for baked designs (designs.json is frontend-only). Options: (a) sync a slug→retail map to the backend (build_catalog.py already builds a catalog — extend it), (b) have `_retail_price_cents_for_slug` fall back to `catalog.json`/a bundled price map, or (c) move all listings into the SUBMISSIONS store. I did NOT hot-patch checkout. No traffic yet = not being exploited.</details>
