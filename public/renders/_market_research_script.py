...
for i in range(5):
    ...
    layer.scale = (size, size, 0.02)
    
    # Bevel
    bevel = layer.modifiers.new(name='Bevel', type='BEVEL')
    bevel.width = 0.001
    bevel.segments = 2
    
    sub = layer.modifiers.new(name='Subdiv', type='SUBSURF')
    sub.levels = 1
    sub.render_levels = 2
...
for i in range(4):
    ...
    bpy.ops.mesh.primitive_cylinder_add(radius=0.003, depth=0.08, location=(x, y, 0.04))
    spike = bpy.context.active_object
    spike.name = f'accent_{i}'
...
nodes.clear()
bsdf = nodes.new('ShaderNodeBsdfPrincipled')
bsdf.inputs['Base Color'].default_value = (0.85, 0.65, 0.15, 1)
...
bg.inputs['Color'].default_value = (0.85, 0.65, 0.15, 1)
