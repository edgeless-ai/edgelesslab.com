...
Usage:
    python notebooklm-bulk-upload.py --channel ColeMedin --notebook <id>
    python notebooklm-bulk-upload.py --channel ColeMedin --topics agentic,ai-tooling --notebook <id>
    python notebooklm-bulk-upload.py --channel ColeMedin --list-topics
"""
...
def load_state() -> Dict:
    """Load upload state (idempotency tracker)."""
...
def discover_articles(channel: str) -> List[KBArticle]:
    """Find all KB articles for a channel."""
...
def filter_by_topics(articles: List[KBArticle], topics: List[str]) -> List[KBArticle]:
    """Keep articles matching ANY requested topic (OR logic)."""
...
def upload_source(article: KBArticle, notebook_id: str, dry_run: bool = False) -> bool:
    """Upload a single article to NotebookLM via CLI."""
...
def run_upload(channel: str, notebook_id: str, topics: Optional[List[str]] = None, dry_run: bool = False):
    ...
    # Check current notebook source count
